...
/** Two-option segmented control (Active On/Off, MRG/Host, etc.). */
export function SegmentedControl<T extends string>({
  ...
  return (
    <div
      className={`flex shrink-0 gap-0.5 rounded-[10px] border border-white/10 bg-[#1a1a1a] p-1 ${
        large ? "min-w-[168px]" : "min-w-[118px]"
      } ${className}`}
    >
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={`flex-1 rounded-[8px] text-center ${
              large
                ? "min-h-[40px] px-4 py-2.5 text-[14.5px]"
                : "min-h-[34px] px-3 py-2 text-[13px]"
            } ${
              active
